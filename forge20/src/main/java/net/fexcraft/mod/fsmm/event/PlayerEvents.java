...
public class PlayerEvents {
    ...
    public static void onPlayerLogin(PlayerEvent.PlayerLoggedInEvent event){
    	...
    	Account account = DataManager.getAccount("player:" + event.getEntity().getGameProfile().getId().toString(), 2);
    	account.addHolder(event.getEntity());
    ...
    public static void onPlayerLogout(PlayerEvent.PlayerLoggedOutEvent event){
		FSMM.LOGGER.info("Unlinking account of " + event.getEntity().getName() + " || " + event.getEntity().getGameProfile().getId().toString());
		DataManager.unholdPlayerAccount(event.getEntity().getGameProfile().getId(), Player.class);
    }
