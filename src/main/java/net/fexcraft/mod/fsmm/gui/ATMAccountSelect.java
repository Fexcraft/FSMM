...
public class ATMAccountSelect extends GenericGui<ATMContainer> {
	...
	public ATMAccountSelect(EntityPlayer player, boolean bool, int[] pos){
	...
	protected void init(){
		...
		this.container.sync("account", mode ? "account_list" : "");
		if(!mode){
	...
	protected void drawlast(float pticks, int mouseX, int mouseY){
		...
		if(type.hovered){
			...
			if(!mode) tooltip.add(Formatter.format("&c&o*required"));
		}
		if(uid.hovered){
